Ext.onReady(function () {
    ...
    var record = new Ext.data.Record.create([
        ...
        {name: 'proxy_ip', mapping: 'proxy_ip'},
        {name: 'proxy_port', mapping: 'proxy_port'},
        {name: 'status', mapping: 'status'}
    ]);

    var proxy = new Ext.data.HttpProxy({
        url: "../../ACLAction_selectControl.action"
    });
    ...
    store.on('load', function () {
        ...
        var proxy_ip = store.getAt(0).get('proxy_ip');
        var proxy_port = store.getAt(0).get('proxy_port');
        Ext.getCmp('acl.url').setValue(url);
        Ext.getCmp('acl.status').setValue(status);
        Ext.getCmp('acl.port').setValue(port);
        Ext.getCmp('acl.proxy_ip').setValue(proxy_ip);
        Ext.getCmp('acl.proxy_port').setValue(proxy_port);
    });
...
//        width:500,
        ...
        items: [
            ...
            }),*/
            {
                ...
                items: [
                    {
                        /*boxLabel: '启用',*/ id: 'acl.status', name: 'status',
                        inputValue: 1
                    }
                ]
            },
                ...
                handler: function () {
                    if (formPanel.form.isValid()) {
                        formPanel.getForm().submit({
                            url: "../../ACLAction_saveConfig.action",
                            method: 'POST',
                            waitTitle: '系统提示',
                            waitMsg: '正在连接...',
                            success: function () {
